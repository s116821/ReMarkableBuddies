...
Reader refreshes a circle while waiting for model responses and shows it at safe page stages. Circle and failure X share a 50 by 50 area near the bottom-right corner. Temporary circle marks are cleared before capture, page navigation, typing and completion; they are never included in model or classifier images. Drawing pauses during keyboard input so pen and keyboard operations cannot interfere.
...
Native status uses the selected drawing tool. The REM-8 hardware gate covers black medium Fineliner and yellow Highlighter with snap-to-text enabled; other tool styles/widths and Paper Pro display behavior are not yet hardware-validated. Stroke paths are inset within the status box to accommodate the tested highlighter footprint.
