...
The initial style UI contract is limited to RM2 firmware 3.28.0.172 in the observed portrait toolbar layout. Unsupported/hidden controls, an already-open pen menu, active non-pen tools or ambiguous metadata suppress status before mutation. Screenshots used to inspect controls have a separate buffer, preserving clean model detail images. Before each dependent UI action the lease checks page/session identity, visible page content and unrelated saved preferences. Menu selection and persisted preferences must converge within five observations. Repeated status ticks never toggle tools.

Before mutation, `/var/cache/reader-buddy/status-style-recovery.json` is created exclusively with owner-private permissions. It stores a bounded versioned original-preference snapshot, without credentials or document text. Normal cleanup and failure marks restore exact preferences and remove the record only after verification. Partial acquisition attempts bounded rollback. Changed pages/layouts or failed restoration stop further tablet input and retain the record. An existing record, even incomplete, is never overwritten: automatic runs stop and report the recovery requirement.

For deliberate recovery after a crash, first stop Reader and any diagnostic writer. Inspect the recovery record, confirm its document and the current user choices, and restore the recorded active slot, primary tool and Fineliner color/width through the native UI. Verify saved preferences and visible controls before removing the recovery record and restarting. Do not apply an old snapshot over subsequent deliberate user changes. Native status ink left by a crash is not automatically claimed or erased.
