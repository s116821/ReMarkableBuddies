...
Every push runs a lightweight full-history path classifier, avoiding GitHub's truncated path-filter behavior. Only application pushes or an explicit retry dispatch enter a single job-level release concurrency group with cancel-in-progress false. Documentation pushes do not enter the release queue or compile. Ordinary main CI similarly gates every application compilation step.

Inside the serialized release job, fetch current main and tags again. Select the latest relevant first-parent main commit after the latest reachable semantic tag. Evaluate the entire unreleased range at that application SHA, excluding docs-only commits; queued application merges can coalesce into one release with the highest applicable semantic bump. A newer documentation HEAD is never substituted as the tag target. Create an annotated managed tag on the selected actual squash SHA and push without force. Confirm the remote tag identity before any application compilation.

Recover all incomplete managed tags before creating/publishing another release. Managed annotation identifies this workflow's tags without pretending older custom releases meet the new runtime contract. Tags are the durable pending-work record. A serialized job checks existing release assets/provenance, rebuilds incomplete releases from their exact tags, stages both architecture packages in a draft and publishes only after verification. Published complete releases are immutable and skipped. API/push/build errors stop visibly; no force updates or version regeneration for the same tagged change. A pending GitHub job can be replaced, but its successor re-reads all unreleased main changes and incomplete tags, so no dependency on one workflow run per commit remains. A manual dispatch retries failure when no new application merge arrives.
